...
async def save_analysis(user_id : str , filename : str , analysis_result : Dict , token) -> Optional[str]:
...
async def get_user_history(user_id :str , token) -> List[Dict]:
    headers = _get_headers(token)
    if not headers:
        return []
...
async def delete_analysis(analysis_id : str , user_id :str , token) -> bool:
    headers = _get_headers(token)
    ...
    try: 
        ...
        logger.error(f"Failes to delete anallysis {analysis_id} : {exc}")
        return False
