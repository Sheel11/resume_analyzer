...
# finds zip code
    ...
    if has_address:
        recommendations.append('Remove full street address - ATS systems dont need this and its a privacy risk ' )
    if has_zip:
...
def validate_skills_with_project(
        skills : List[str],
        projects : List[Dict],
...
def _calc_formatting_score(parsed_resume : Dict , text : str) -> float:
    ...
    exp_entries = [e for e in parsed_resume.get('expereince' , []) if isinstance(e , dict)]
    ...
    proj_entries  = [p for p in parsed_resume.get('project' , []) if isinstance(p,dict)]
