...
def _extract_pdf_with_pdfplumber(file_data : bytes) -> str :
    ...
    if not text.strip():
        raise TextExtractionError(
            ...
            user_message='No text could be exxtracted from the PDF.'
        )
...
def _extract_pdf_with_pydf2(file_data : bytes) -> str:
    text = ''
...
def extract_text_from_pdf(file_data :bytes) ->str :
    try:
        result, used_fallback = with_fallback(
            ...
            _extract_pdf_with_pydf2,
            file_data,
            log_fallback=True
        )

        if used_fallback:
            log_info('PDF EXTRACTION succeded using the PyPDF2 fallback' , context='resume parser')
            return result
        return result
    except Exception as e:
        ...
        raise FileParsingError(
            ...
            'The PDF may be corruted, password-protected , or contain only scanned images. '
    ...
    try :
        ...
        try:
            for rel in doc.part.rels.values():
                if 'hyerlink' in rel.reltype.lower():
                    url = rel._target
...
def parse_resume_file(file_data : bytes , filename : str) -> Tuple[str , dict]:
    ...
    try:
        ...
        raise FileValidationError(
            'Could not validate the uploaded file. Please ensure it is a valid PDF or DOCS. '
        ) from e
    

    #p-2 : extraction of file
