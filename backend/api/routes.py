...
async def analyze_resume(
    ...
    job_description : str = Form('' , description = 'Job description text (optinal)'),
    ...
    # extract jd_comparision details 
    jd_comparison_result = None
    if result.get('jd_comaprison'):
    ...
    detailed_fd = result.get('detailed_feedback', [])
    ...
    try:
        from backend.db.supabase_db import save_analysis
        _ , token = authorization.split()
        await save_analysis(user_id , filename , result , token)
    except Exception as exc :
...
@router.get('/history')
async def get_history(user_id: str = Depends(get_current_user) , authorization : str = Header(...)):
    """Return the signed-in user's past analyses (identity comes from the JWT)."""
    from backend.db.supabase_db import get_user_history
    try:
        _ , token = authorization.split()
        return await get_user_history(user_id , token)
    except Exception as exc:
...
async def delete_history_entry(
    ...
    _ , token = authorization.split(' ')
    from backend.db.supabase_db import delete_analysis
    try:
        success = await delete_analysis(id, user_id , token)
        if not success:
...
async def generate_history_pdf(
    ...
    _ , token = authorization.split()
    history = await get_user_history(user_id , token)
